...
const deployDemoApp = async (
  ...
  const approvalProgram = await getBasicProgramBytes(
    `../../artifacts/${approvalFileName}`
  );
  const clearProgram = await getBasicProgramBytes(
    `../../artifacts/${clearStateFileName}`
  );

  // global / local states
  const numGlobalInts = states[0];
  const numGlobalByteSlices = states[1];
  const numLocalInts = states[2];
  const numLocalByteSlices = states[3];
